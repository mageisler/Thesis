...
Signal efficiency and purity have already been defined in Section~\ref{sec:AMWFTW}. While the efficiency as defined in Equation~\ref{eq:Efficiency} is kept, the purity as in Equation~\ref{eq:Purity} is slightly modified to account only for association tasks:
\begin{equation}
...
\begin{figure}[!ht]
    ...
    \caption[Efficiencies, purities and their product of the different qualities of the association map with associating to the first vertex always in step 3]{The efficiencies, purities and the product of both of the different qualities for associating the track to the first vertex in step 3. The distributions are shown as a function of the pseudorapidity $\eta$ (left hand plots), transverse momentum (middle plots) and the number of pileup interactions (right hand plots). \label{plot:TASEFRDQ1Signal}}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \caption[Efficiencies, purities and their product for the different qualities of the association map with associating to the closest vertex in z only in step 3]{The efficiencies, purities and the product of both of the different qualities for associating the track to the closest vertex in z only in step 3. The distributions are shown as a function of the pseudorapidity $\eta$ (left hand plots), transverse momentum (middle plots) and the number of pileup interactions (right hand plots). \label{plot:TASEFRDQZSignal}}
\end{figure}

In Table~\ref{tab:TAQualComp} the averaged values are shown for all qualities. The resulting efficiencies and purities for this option do not differ much from associating the track to the closest vertex in all three dimension during step 3. Therefore, the discussion given earlier is also valid here.
...
\begin{figure}[!ht]
    ...
    \caption[Pileup efficiencies, purities and their product for the different qualities of the association map with associating to the first vertex always in step 3]{The pileup efficiencies, purities and the product of both of the different qualities for associating the track to the first vertex in step 3. The distributions are shown as a function of the pseudorapidity $\eta$ (left hand plots), transverse momentum (middle plots) and the number of pileup interactions (right hand plots). \label{plot:TASEFRDQ1Pileup}}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \caption[Pileup efficiencies, purities and their product for the different qualities of the association map with associating to the closest vertex in three dimensions in step 3]{The pileup efficiencies, purities and the product of both of the different qualities of the association map with associating to the closest vertex in three dimensions in step 3. The distributions are shown as a function of the pseudorapidity $\eta$ (left hand plots), transverse momentum (middle plots) and the number of pileup interactions (right hand plots). \label{plot:TASEFRDQ3Pileup}}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \caption[Pileup efficiencies, purities and their product for the different qualities of the association map with associating to the closest vertex in z only in step 3]{The pileup efficiencies, purities and the product of both of the different qualities of the association map with associating to the closest vertex in z only in step 3. The distributions are shown as a function of the pseudorapidity $\eta$ (left hand plots), transverse momentum (middle plots) and the number of pileup interactions (right hand plots). \label{plot:TASEFRDQZPileup}}
\end{figure}
...
In summary, in most cases the association map leads to the best performance regarding the combination of efficiency and purity. Only for tracks with a high \pt{} applying no pileup subtraction leads to better results. For instance, for charged hadrons for tracks with a \pt{} greater than 10\GeV{} this diference is in the order of $5\,\%$. Thus, for these particles the performance of applying no pileup subtraction is better in terms of pileup efficiency and purity for high \pt{}, too. For all other cases the association map leads to the best results regarding the performance in subtracting pileup tracks. \\
As for most analyses, a common method is desired for all kind of objects, it is proposed to use the association map globally.
